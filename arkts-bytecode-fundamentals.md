...
In Ark Bytecode, in addition to register and accumulator, there are four value storage modes: **global variable**, **[module] namespace and variable**, **lexical environment and variable**, and **patch variable**. The instruction can use the values stored by these four modes as input parameters.
...
### Value Storage Mode
#### Global variables
In [Script] build mode, a global variable is a variable stored in a globally unique mapping. Its key is the name of the global variable, and its value is that of the global variable. Global variables can be accessed through global-related instructions.<br>
...
All [module namespaces] used in the source file are compiled into an array. An index is used in the instruction to reference a module namespace. For example, *getmodulenamespace 0x1* references the module namespace at the *0x1* index.<br>
...
The scenarios for generating module instructions include [import] and [export]. The main scenarios are as follows:
...
For a method that contains N pieces of formal parameters, the last N+3 registers used by the method are used to pass parameters. In this case, the first three registers represent the function (FunctionObject) itself, [new.target] (NewTarget), and **this** in the lexical environment. The subsequent N registers correspond to these N pieces of formal parameters.<br>
...
|  0x1d	|  IMM8	|  tonumber RR	|  Default input parameter: acc: object<br>R: 8-bit reserved number used in Ark runtime|  Use **acc** as a parameter, execute [ToNumber], and store the result in **acc**.  |
|  0x1e	|  IMM8	|  tonumeric RR	|  Default input parameter: acc: object<br>R: 8-bit reserved number used in Ark runtime|  Use **acc** as a parameter, execute [ToNumeric], and store the result in **acc**.  |
...
|  0x36	|  V8	|  getnextpropname vAA	| A: iterator|  Execute the [next] method of [for-in iterator] A and store the result in **acc**.  |
...
|  0x66	|  NONE	|  getpropiterator	|  Default input parameter: acc: object|  Stores the [for-in iterator] of the object in **acc**.  |
|  0x67	|  IMM8	|  getiterator RR	|  Default input parameter: acc: object<br>R: 8-bit reserved number used in Ark runtime|  Execute the [GetIterator] (acc, sync) method and store the result in **acc**.  |
|  0x68	|  IMM8_V8	|  closeiterator RR, vAA	|  R: 8-bit reserved number used in Ark runtime<br>A: Object|  Use A of the *[iteratorRecord]* type as a parameter to execute [IteratorClose] and store the result in **acc**.  |
...
|  0x76	|  IMM8	|  gettemplateobject RR	|  Default input parameter: acc: object<br>R: 8-bit reserved number used in Ark runtime|  Execute [GetTemplateObject] (acc) and store the result in **acc**.  |
...
|  0x7b	|  IMM8	|  getmodulenamespace +AA	|  A: module index|  Execute the [GetModuleNamespace] instruction for module A and store the result in **acc**.  |
...
|  0x97	|  V8	|  asyncgeneratorreject vAA	 |  Default input parameter: acc: exception<br>A: generator|  Use the exception stored in *[generator]* A and **acc**, execute [AsyncGeneratorReject], and store the result in **acc**.  |
...
|  0xab	|  IMM16	|  getiterator RRRR	|  Default input parameter: acc: object<br>R: 16-bit reserved number used in Ark runtime|  Execute the [GetIterator] (acc, sync) method and store the result in **acc**.  |
|  0xac	|  IMM16_V8	|  closeiterator RRRR, vAA	|  R: 16-bit reserved number used in Ark runtime<br>A: Object|  Use A of the *[iteratorRecord]* type as a parameter to execute [IteratorClose] and store the result in **acc**.  |
...
|  0xb2	|  V8_V8	|  createiterresultobj vAA, vBB	|  A: Object<br>B: Boolean value|  Set *value* A and *done* B as parameters to execute [CreateIterResultObject] instruction and store the result in **acc**.  |
...
|  0xb8	|  V8_V8_V8	|  asyncgeneratorresolve vAA, vBB, vCC	|  A: generator<br>B: object<br>C: boolean value|  Use *generator* A, *value* B, and *done* C as parameters to execute [AsyncGeneratorResolve] and store the result in **acc**.  |
...
|  0xbd	|  NONE	|  dynamicimport	|  Default input parameter: acc: value|  Use the value of **acc** as a parameter to execute [ImportCalls] and store the result in **acc**.  |
...
|  0xbf	|  NONE	|  resumegenerator	|  Default input parameter: acc: generator|  Execute [GeneratorResume] based on the generator stored in **acc** and store the result in **acc**.  |
...
|  0xc1	|  IMM16	|  gettemplateobject RRRR	|  Default input parameter: acc: object<br>R: 16-bit reserved number used in Ark runtime|  Execute [GetTemplateObject] (acc) and store the result in **acc**.  |
...
|  0xc4	|  V8	|  asyncfunctionawaituncaught vAA	|  Default input parameter: acc: value<br>A: function object|  Use the function object A and the value of **acc** to execute [AwaitExpression] and store the result in **acc**.  |
...
|  0xc6	|  V8_V8	|  starrayspread vAA, vBB	|  Default input parameter: acc: value<br>A: array<br>B: array index|  Store the value in **acc** to the position starting with index B of array A in the format of [SpreadElement], and store the length of the result array in **acc**.  |
...
|  0xcf	|  IMM8	|  copyrestargs +AA	|  A: position of the [rest parameter] in the formal parameter list|  Copy the rest parameters and store the parameter array copy in **acc**.  |
...
|  0xd6	|  IMM8	|  setgeneratorstate +AA	|  Default input parameter: acc: generator object<br>A: generator state|  Set the generator state stored in **acc** to A. For details, see [GeneratorState] and [AsyncGeneratorState].<br>A may have the following values: undefined(0x0), suspendedStart(0x1), suspendedYield(0x2), executing(0x3), completed (0x4), and awaitingReturn (0x5).  |
|  0xd7	|  IMM8	|  getasynciterator RR	|  Default input parameter: acc: object<br>R: 8-bit reserved number used in Ark runtime|  Execute the [GetIterator] (acc, async) and store the result in **acc**.  |
...
|  0x0efd	|  PREF_IMM16	|  wide.getmodulenamespace +AAAA	|  A: module index|  Execute the [GetModuleNamespace] instruction for module A and store the result in **acc**.  |
